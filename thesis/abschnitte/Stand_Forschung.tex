...
Besonders die Diversität ist eine erwünschte Eigenschaft, die eine andere Formulierung des Selektion benötigt.
...
Die Crossover$\rightarrow$Mutation$\rightarrow$Selektion Schleife wird so oft wiederholt werden bis entweder eine Abbruchkriterium, wie eine zu niedrige Verbesserungsrate, oder die maximale Anzahl an Generationen erreicht ist.
...
Dadurch wird automatisch eine Aussage über die eigene Sicherheit bezüglich der Vorhersage getroffen, wodurch fehlerhafte Aussagen, die andere Machine Learning Verfahren weit entfern von Trainingsdaten treffen vermieden werden können.
...
Das einfachste Beispiel in dem eine solche Spannung existiert ist das Problem des mehrarmigen Banditen \todo{zitat}.
...
Die Varianz spiegelt die Unsicherheit an einem Punkt wieder, eine Auswahl nach Varianz bevorzugt die Exploration von Bereichen in denen eine hohe Varianz auftritt, also eben solche Bereich in denen noch keine Datenpunkte existieren.
...
Mit diese Gaußprozess wird dann ein letztes mal MAP-Elites durchgeführt, statt der Akquisefunktion wird allerdings die Fitnessfunktion optimiert, die durch den Mittelwert des Gaußprozesses beschrieben wird.
...
Es muss folglich zuerst eine mathematische Formulierung für den Constraint entwickelt werden, die dem Constraint soweit wie möglich entspricht.
Zweitens werden Constraints in einem typischen evolutionären Optimierungsalgorithmus sehr häufig evaluiert werden. 
Das bedeutet, dass die Berechnung des Constraints entweder nicht besonders rechenintensiv sein darf, oder Hilfsmechanismen integriert werden wie beispielsweise die Modellierung des Constraint durch ein Surrogatnetzwerk, wie es in SAIL für die Fitness bereits geschieht.
